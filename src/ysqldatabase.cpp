#include "ysqldatabase.h"


YSqlDatabase* YSqlDatabase::m_instance = NULL;

YSqlDatabase::YSqlDatabase(const QString &data_path)
{
    m_database = QSqlDatabase::addDatabase("QSQLITE");
    QFileInfo _file_info(data_path);

    if(_file_info.isFile())
    {
        m_database.setDatabaseName(data_path);
        if (!m_database.open())
        {
            QMessageBox::information(NULL, "error", "dataset open error.");
        }
        else
        {

            qDebug() << "dataset open sucess";
        }
    }
    else
    {
        qDebug() << "dataset not exist";
    }
    init_map();

}

YSqlDatabase::~YSqlDatabase()
{
    delete m_instance;
    m_database.close();
}

void YSqlDatabase::init_map()
{
    NameMap["ipc_id"] = "IPC编号";
    NameMap["ip"] = "IP地址";
    NameMap["mac"] = "MAC地址";
    NameMap["rtsp_main_stream"] = "主码流地址";
    NameMap["rtsp_sub_stream"] = "子码流地址";
    NameMap["worker_id"] = "员工编号";
    NameMap["name"] = "姓名";
    NameMap["sex"] = "性别";
    NameMap["age"] = "年龄";
    NameMap["note"] = "备注";
    NameMap["conn_ipc_id"] = "IPC编号";
    NameMap["conn_worker_id"] = "员工编号";
    NameMap["begin_time"] = "工作起始时间";
    NameMap["end_time"] = "工作结束时间";
}

void YSqlDatabase::load_database_by_table_name(QString &table_name, QSqlQueryModel *query_model)
{
    QString sql = "select * from " + table_name;
    query_model->setQuery(sql);
}

void YSqlDatabase::convert_stringName(QSqlQueryModel *query_model, std::vector<QString> &column_names)
{
    for(int i=0;i<query_model->columnCount();i++){

        auto headstring = NameMap.find(query_model->headerData(i,Qt::Horizontal).toString());
        if(headstring!=NameMap.end()){
            column_names.push_back(headstring.value());

        }
        else{
            column_names.push_back(query_model->headerData(i,Qt::Horizontal).toString());
        }
     }

}

bool YSqlDatabase::init_tableView(QString table_name, QTableView *table_view, QStandardItemModel *table_model, int cols)
{

    table_view->setEditTriggers(QAbstractItemView::NoEditTriggers);


    QString sql = "select * from " + table_name;
    QSqlQueryModel *query_model =new QSqlQueryModel();
    query_model->setQuery(sql);


    table_view->setModel(table_model);
    table_model->clear();

    table_model->setColumnCount(cols);

    std::vector<QString> column_names;


    convert_stringName(query_model,column_names);

    for(int table_head_index = 0; table_head_index < cols; table_head_index++)
    {
        table_model->setHeaderData(table_head_index, Qt::Horizontal, column_names[table_head_index]);
    }


    table_view->resizeColumnsToContents();   //根据内容设置列宽
    table_view->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
//    table_view->verticalHeader()->setSectionResizeMode(QHeaderView::Fixed);  //列宽不可更改
    table_view->horizontalHeader()->setMinimumSectionSize(100);  //设置表最小列宽

    table_view->verticalHeader()->hide();   //隐藏最左侧栏
    table_view->horizontalHeader()->setStretchLastSection(true);  //最后一列填满
    table_view->setAlternatingRowColors(true);    //奇偶行不同颜色
    table_view->setSelectionBehavior(QAbstractItemView::SelectRows);  //一次选中整行
    table_view->setSelectionMode(QAbstractItemView::SingleSelection);  //一次只选一行
    table_view->setContextMenuPolicy(Qt::CustomContextMenu);

    for(int j = 0;j < query_model->rowCount();j++)
    {
        table_model->setItem(j, 0, new QStandardItem(QString::number(j+1)));
        table_model->item(j, 0)->setTextAlignment(Qt::AlignCenter);
        for(int k=0;k<cols;k++)
        {

            table_model->setItem(j,k,new QStandardItem(query_model->record(j).value(k).toString()));  //动态添加行
            table_model->item(j,k)->setTextAlignment(Qt::AlignCenter); //设置表内容居中

        }
    }
    return true;

}

bool YSqlDatabase::init_tableView(QString table_name, QTableView *table_view, QStandardItemModel *table_model)
{

    table_view->setEditTriggers(QAbstractItemView::NoEditTriggers);


    QString sql = "select * from " + table_name;
    QSqlQueryModel *query_model =new QSqlQueryModel();
    query_model->setQuery(sql);


    table_view->setModel(table_model);
    table_model->clear();

    table_model->setColumnCount(query_model->columnCount() + 1);

    std::vector<QString> column_names;


    convert_stringName(query_model,column_names);
    int table_head_index = 1;
    table_model->setHeaderData(0, Qt::Horizontal, QString("序号"));
    for(auto n:column_names){
        table_model->setHeaderData(table_head_index++, Qt::Horizontal, n);
    }


    table_view->resizeColumnsToContents();   //根据内容设置列宽
    table_view->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
//    table_view->verticalHeader()->setSectionResizeMode(QHeaderView::Fixed);  //列宽不可更改
    table_view->horizontalHeader()->setMinimumSectionSize(100);  //设置表最小列宽

    table_view->verticalHeader()->hide();   //隐藏最左侧栏
    table_view->horizontalHeader()->setStretchLastSection(true);  //最后一列填满
    table_view->setAlternatingRowColors(true);    //奇偶行不同颜色
    table_view->setSelectionBehavior(QAbstractItemView::SelectRows);  //一次选中整行
    table_view->setSelectionMode(QAbstractItemView::SingleSelection);  //一次只选一行
    table_view->setContextMenuPolicy(Qt::CustomContextMenu);

    for(int j=0;j<query_model->rowCount();j++)
    {
        table_model->setItem(j, 0, new QStandardItem(QString::number(j+1)));
        table_model->item(j, 0)->setTextAlignment(Qt::AlignCenter);
        for(int k=1;k<query_model->columnCount() + 1;k++)
        {

            table_model->setItem(j,k,new QStandardItem(query_model->record(j).value(k-1).toString()));  //动态添加行
            table_model->item(j,k)->setTextAlignment(Qt::AlignCenter); //设置表内容居中

        }
    }
    return true;

}

bool YSqlDatabase::init_tableView(std::vector<QString> table_head, QTableView *table_view, QStandardItemModel *table_model)
{
    table_view->setModel(table_model);
    table_model->clear();
    table_view->setEditTriggers(QAbstractItemView::NoEditTriggers);


    table_model->setColumnCount(int(table_head.size()));

    int table_head_index = 0;
    for(auto n : table_head){
        table_model->setHeaderData(table_head_index++, Qt::Horizontal, n);
    }

    table_view->resizeColumnsToContents();
    table_view->horizontalHeader()->setSectionResizeMode(QHeaderView::ResizeToContents);
    table_view->verticalHeader()->setSectionResizeMode(QHeaderView::Fixed);
    table_view->horizontalHeader()->setMinimumSectionSize(200);

    table_view->verticalHeader()->hide();
    table_view->horizontalHeader()->setStretchLastSection(true);
    table_view->setAlternatingRowColors(true);
    table_view->setSelectionBehavior(QAbstractItemView::SelectRows);
    table_view->setSelectionMode(QAbstractItemView::SingleSelection);
    table_view->setContextMenuPolicy(Qt::CustomContextMenu);

    table_view->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);

    return true;
}



bool YSqlDatabase::is_exist_id_database(QString &id, QString &id_type, QString &database_type)
{
    QSqlQuery query;
    QString sql = "select * from " + database_type + " where "+ id_type + " = '" + id + "'";
    query.exec(sql);
    return query.next();
}

bool YSqlDatabase::export_to_excel(QString fileName, QString sheetName, QString title, QString columnNames[], int columnWidths[], int columnCount, QStringList content)
{
    //逐个拼接xml字符,再保存为xls文件
    QStringList html;

    //固定头部信息
    html.append("<?xml version=\"1.0\"?>\n");
    html.append("<?mso-application progid=\"Excel.Sheet\"?>\n");
    html.append("<Workbook xmlns=\"urn:schemas-microsoft-com:office:spreadsheet\"\n");
    html.append(" xmlns:o=\"urn:schemas-microsoft-com:office:office\"\n");
    html.append(" xmlns:x=\"urn:schemas-microsoft-com:office:excel\"\n");
    html.append(" xmlns:ss=\"urn:schemas-microsoft-com:office:spreadsheet\"\n");
    html.append(" xmlns:html=\"http://www.w3.org/TR/REC-html40\">\n");

    //文档信息
    html.append(" <DocumentProperties xmlns=\"urn:schemas-microsoft-com:office:office\">\n");
    html.append("  <LastAuthor>安防周界报警系统</LastAuthor>\n");
    html.append(QString("  <Created>%1</Created>\n").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd HH:mm:ss")));
    html.append("  <Version>11.6360</Version>\n");
    html.append(" </DocumentProperties>\n");

    html.append(" <OfficeDocumentSettings xmlns=\"urn:schemas-microsoft-com:office:office\">\n");
    html.append("  <AllowPNG/>\n");
    html.append(" </OfficeDocumentSettings>\n");

    html.append(" <ExcelWorkbook xmlns=\"urn:schemas-microsoft-com:office:excel\">\n");
    html.append("  <WindowHeight>9795</WindowHeight>\n");
    html.append("  <WindowWidth>21435</WindowWidth>\n");
    html.append("  <WindowTopX>120</WindowTopX>\n");
    html.append("  <WindowTopY>75</WindowTopY>\n");
    html.append("  <WindowTopY>75</WindowTopY>\n");
    html.append("  <ProtectWindows>False</ProtectWindows>\n");
    html.append(" </ExcelWorkbook>\n");

    //样式表
    html.append(" <Styles>\n");
    html.append("  <Style ss:ID=\"Default\" ss:Name=\"Normal\">\n");
    html.append("   <Alignment ss:Vertical=\"Center\"/>\n");
    html.append("   <Borders/>\n");
    html.append("   <Font ss:FontName=\"宋体\" x:CharSet=\"134\" ss:Size=\"11\" ss:Color=\"#000000\"/>\n");
    html.append("   <Interior/>\n");
    html.append("   <NumberFormat/>\n");
    html.append("   <Protection/>\n");
    html.append("  </Style>\n");
    //标题样式
    html.append("  <Style ss:ID=\"s64\">\n");
    html.append("   <Alignment ss:Horizontal=\"Center\" ss:Vertical=\"Center\"/>\n");
    html.append("   <Font ss:FontName=\"宋体\" x:CharSet=\"134\" ss:Size=\"14\" ss:Color=\"#000000\" ss:Bold=\"1\"/>\n");
    html.append("  </Style>\n");
    //带边框样式
    html.append("  <Style ss:ID=\"s91\">\n");
    html.append("  <Borders>\n");
    html.append("   <Border ss:Position=\"Bottom\" ss:LineStyle=\"Continuous\" ss:Weight=\"1\"/>\n");
    html.append("   <Border ss:Position=\"Left\" ss:LineStyle=\"Continuous\" ss:Weight=\"1\"/>\n");
    html.append("   <Border ss:Position=\"Right\" ss:LineStyle=\"Continuous\" ss:Weight=\"1\"/>\n");
    html.append("   <Border ss:Position=\"Top\" ss:LineStyle=\"Continuous\" ss:Weight=\"1\"/>\n");
    html.append("  </Borders>\n");
    html.append(" </Style>\n");

    html.append(" </Styles>\n");

    //工作表名称
    html.append(QString(" <Worksheet ss:Name=\"%1\">\n").arg(sheetName));
    int rowCount = content.count(); //计算总行数

    //表格开始
    html.append(QString("  <Table ss:ExpandedColumnCount=\"%1\" ss:ExpandedRowCount=\"%2\" x:FullColumns=\"1\"\n").arg(columnCount).arg(rowCount + 2));
    html.append("   x:FullRows=\"1\" ss:DefaultColumnWidth=\"54\" ss:DefaultRowHeight=\"18\">\n");

    //设置字段宽度
    for (int i = 0; i < columnCount; i++) {
        html.append(QString("   <Column ss:AutoFitWidth=\"0\" ss:Width=\"%1\"/>\n").arg(QString::number(columnWidths[i])));
    }

    //表格标题
    html.append("   <Row ss:AutoFitHeight=\"0\" ss:Height=\"26.25\">\n");
    html.append(QString("    <Cell ss:MergeAcross=\"%1\" ss:StyleID=\"s64\"><Data ss:Type=\"String\">%2</Data></Cell>\n").arg(columnCount - 1).arg(title));
    html.append("   </Row>");

    //逐个添加字段名称
    html.append("   <Row ss:AutoFitHeight=\"0\">\n");
    for (int i = 0; i < columnCount; i++) {
        html.append("    <Cell ss:StyleID=\"s91\"><Data ss:Type=\"String\">" + columnNames[i] + "</Data></Cell>\n");
    }
    html.append("   </Row>\n");

    //逐个添加数据
    for (int i = 0; i < rowCount; i++) {
        html.append("   <Row ss:AutoFitHeight=\"0\">\n");
        QString temp = content[i];
        QStringList value = temp.split(";");
        foreach (QString str, value) {
            html.append("    <Cell ss:StyleID=\"s91\"><Data ss:Type=\"String\">" + str + "</Data></Cell>\n");
        }
        html.append("   </Row>\n");
    }
    html.append("  </Table>\n");

    //固定结尾格式
    html.append("  <WorksheetOptions xmlns=\"urn:schemas-microsoft-com:office:excel\">\n");
    html.append("   <PageSetup>\n");
    html.append("    <Header x:Margin=\"0.3\"/>\n");
    html.append("    <Footer x:Margin=\"0.3\"/>\n");
    html.append("    <PageMargins x:Bottom=\"0.75\" x:Left=\"0.7\" x:Right=\"0.7\" x:Top=\"0.75\"/>\n");
    html.append("   </PageSetup>\n");
    html.append("   <Unsynced/>\n");
    html.append("   <Selected/>\n");
    html.append("   <ProtectObjects>False</ProtectObjects>\n");
    html.append("   <ProtectScenarios>False</ProtectScenarios>\n");
    html.append("  </WorksheetOptions>\n");
    html.append(" </Worksheet>\n");
    html.append("</Workbook>\n");

    //写入文件
    QFile file(fileName);
    if (file.open(QFile::WriteOnly | QIODevice::Text)) {
        QTextStream text(&file);
        text.setCodec("UTF-8");
        text << html.join("");
    }

    return true;
}

